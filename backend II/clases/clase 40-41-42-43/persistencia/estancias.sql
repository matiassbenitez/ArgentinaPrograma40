...
select * from casas where fecha_desde <= '2020-08-01' and fecha_hasta >= adddate(fecha_desde, interval 30 day);
select clientes.*, (select tipo_vivienda from casas where casas.id_casa = estancias.id_casa) from estancias inner join clientes on estancias.id_cliente = clientes.id_cliente;
select estancias.*, (select * from casas where casas.id_casa = estancia_cliente.id_casa)  from estancias inner join clientes as estancia_cliente on estancias.id_cliente = clientes.id_cliente ;
select * from casas left join comentarios on casas.id_casa = comentarios.id_casa where comentario like "%limpia%";